...
public class ViewPagerFragChicken extends Fragment {
    private RecyclerView chicken_recyclerview;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_view_pager_chicken, container, false);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        chicken_recyclerview = view.findViewById(R.id.chicken_recyclerview);

        // Inflate the layout for this fragment
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getContext());
        chicken_recyclerview.setLayoutManager(linearLayoutManager);

        itemObjects = new ArrayList<>();
        mainRecyclerAdapter = new MainRecyclerAdapter(getContext(),itemObjects);
        chicken_recyclerview.setAdapter(mainRecyclerAdapter);

        //리사이클러뷰 높이 여백 지정.
        spaceDecoration = new RecyclerViewDecoration(30);
        chicken_recyclerview.addItemDecoration(spaceDecoration);

        //리사이클러뷰 구분선 추가.
        DividerItemDecoration dividerItemDecoration =
                new DividerItemDecoration(chicken_recyclerview.getContext(),new LinearLayoutManager(getContext()).getOrientation());
        chicken_recyclerview.addItemDecoration(dividerItemDecoration);
        ...
        if(permissionResult == PackageManager.PERMISSION_GRANTED){  //권한이 있으면
            new SearchTask(localRead + "치킨", page, mainRecyclerAdapter, itemObjects).execute();
        }else if(ActivityCompat.shouldShowRequestPermissionRationale(getActivity(), Manifest.permission.INTERNET)){  //권한 요청화면을 띄워줌
            new SearchTask(localRead + "치킨", page, mainRecyclerAdapter, itemObjects).execute();    //권한 허락이 되었을 때 실행
        }

        chicken_recyclerview.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                super.onScrollStateChanged(recyclerView, newState);
                if (!chicken_recyclerview.canScrollVertically(1)){
                    page += 1;
                    new SearchTask(localRead+"치킨", page, mainRecyclerAdapter, itemObjects).execute();
                }
//                else if (!chicken_recyclerview.canScrollVertically(-1)){
//                    page = 1;
//                    new SearchTask(localRead+"치킨",chicken_recyclerview, page, mainRecyclerAdapter, itemObjects).execute();
//                }
            }
        });
    }
